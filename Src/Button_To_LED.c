/*  AIM:
    Keep the Internal LED on till a Push Button is pushed.
    ...
    Check the input pin, and when found low, turn on the internal LED connected to PB5 till the button is released.
*/
...
int main()
{
    ...
    while (1)
    {
        if (check_DeBounce(PORTD, PIN2))
        {
            // If button is pressed, turn on the internal LED.
